...
std::pair<double,double> stFilters::filtering(int& x,int& y,double& theta,double& time,double& phase){
     ...
     //std::cout << " " << dx << " " << dy << " " << theta << " "<< phase << " " << time << std::endl;//Debug Code
     ...
     double f1 = (f_spatial * dx * cos(theta) + f_spatial * dy * sin(theta) );
     double f2 = (f_spatial * dx * cos(theta) + f_spatial * dy * sin(theta) );
     double even_spatial = D * gaussian_spatial  * cos( (2 * M_PI * f1 ) + phase );
     double odd_spatial = D * gaussian_spatial * sin( (2 * M_PI * f2) + phase );
